...
def _easter_regressor(index: pd.DatetimeIndex, k: int = 8) -> pd.Series:
    ...
    for p in periods.unique():
        ...
        easter_date = pd.Timestamp(p.year) + Easter()
...
def seasonal_adjust(
    ...
    *,
    ...
    if (method in ("auto", "x13")) and _HAS_X13:
        try:
            x13_res = x13_arima_analysis(resid.dropna(), x12path=r"C:\\ONSapps\\My_Spyder\\Library\\bin\\x13as_ascii.exe")
            ...
            raise #chosen_method = None

    # STL fallback
    if chosen_method is None:
        print("X-13ARIMA-SEATS failed, using STL fallback")
